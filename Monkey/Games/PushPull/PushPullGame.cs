...
using Monkey.Games.PushPull.Notification;
...
{
    public class PushPullGame : GameBase<PushPullHub>, IGameUpdate
    {
        readonly log4net.ILog logger = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public PushPullGame(string name, string viewPath, int maxPlayers, Player[] players, Dictionary<string, object> props)
            : base(name, viewPath, maxPlayers, players, props)
        {

            if (logger.IsInfoEnabled)
                logger.InfoFormat("Push Pull Game Initialized. Name: [{0}]", name);
        ...
        private bool IsActivePlayer(PushPullPlayer player)
        {
        ...
        private PushPullPlayer GetNextPlayer()
        {
        ...
        /// </summary>
        /// <param name="player"></param>
        private void StartTurn(PushPullPlayer player)
        {
            if (logger.IsInfoEnabled)
        ...
        {
            this.players = new PushPullPlayer[players.Length];

            var random = new Random();
            for (var i = 0; i < players.Length; i++)
            {
                players[i].GamePlayer = new PushPullPlayer(this, players[i]);
                this.players[i] = (PushPullPlayer)players[i].GamePlayer;
            }
        ...
        /// List of the players
        /// </summary>
        private PushPullPlayer[] players;
